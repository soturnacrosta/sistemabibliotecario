...
Sprint Backlog: Nesse Sprint serão desenvolvidas as funções básicas do software: registro e consulta de livros. Para isso, serão feitos estudos sobre alocação dinâmica na internet e uso de IA para tal. Os itens do Product Backlog serão: 
...
Incrementos: - Registrar livros entregue funcionalmente; 

		- Consultar livros entregue funcionalmente; 

		- Atualizar livros entregue funcionalmente. 

Sprint Review: As funções de registrar livros e consultá-los foram devidamente implementadas. O uso de alocação dinâmica e ponteiros foram bem sucedidas até esse ponto. Foi entregue a função de atualizar cadastros com erros. O uso de arquivo binário e structs também foi entregue. 

Sprint Retrospective: A Sprint somou bastante conhecimento. A utilização de alocação dinâmica era obscura até então e utilizá-lo em um projeto clareou bastante as coisas. Embora seja difícil e com muitos erros de controle de memória, torna o programa mais limpo.  
...
Sprint Backlog: Esse Sprint tem como objetivo entregar as funções de atualização e remoção de livros, bem como correção de erros. Para tal, será utilizado canais de pesquisa online como StakOverflow e IA. Os itens do Product Backlog são: 
...
Sprint Review: Essa Sprint teve como maior resultado o lançamento da versão 1.0 do Sistema Bibliotecário. Para isso, as funções de atualizar e remover cadastro foram implementadas funcionalmente. Os problemas de gerenciamento de memória foram quase todos erradicados. Atendidas os requisitos de utilização de ponteiros, alocação dinâmica, structs e arquivo binário. 

Sprint Retrospective: Os maiores problemas encontrados foram no gerenciamento de memória, através de diagnósticos do Valgrind e ADRESS SINTETIZER. Houve enormes avanços quantos esses leaks na memória, mas restou uma (1) mensagem de erro a ser revisada. Mas o programa encontra-se funcional, com resultado satisfatório. 
